...
class StopTimeController(Controller):
    ...
    @get("/trip/{trip_id:str}", summary="Stop times by trip ID", raises=[NotFoundException])
    async def get_stop_time_by_trip_id(
        ...
        if results is None or len(results) == 0:
            raise NotFoundException(detail=f"StopTimes not found for trip id {trip_id}")
        return [StopTime.model_validate(obj) for obj in results]

    @get("/stop/{stop_id:str}", summary="Stop times by stop ID", raises=[NotFoundException])
    async def get_stop_time_by_stop_id(
        ...
        if results is None or len(results) == 0:
            raise NotFoundException(detail=f"StopTimes not found for stop id {stop_id}")
        return [StopTime.model_validate(obj) for obj in results]
