...
from SimplyTransport.api_contracts.agency_contracts import Agency, AgencyWithTotal

from ...domain.agency.agency_repo import AgencyRepo, provide_agency_repo
...
class AgencyController(Controller):
    ...
    async def get_all_agencies(self, repo: NamedDependency[AgencyRepo]) -> list[Agency]:
    ...
    async def get_all_agencies_and_count(self, repo: NamedDependency[AgencyRepo]) -> AgencyWithTotal:
    ...
    async def get_agency_by_id(self, repo: NamedDependency[AgencyRepo], id: FromPath[str]) -> Agency:
        try:
