...
from .stop_model import StopModel


class StopRepo(SQLAlchemyAsyncRepository[StopModel]):  # type: ignore[type-var]
    """Stop repository."""
...
async def provide_stop_repo(db_session: NamedDependency[AsyncSession]) -> StopRepo:
    """This provides the Stop repository."""

    return StopRepo(session=db_session)
