...
def _pick_predecessor_carry_forward(
    ...
    if not predecessors:
        return None
    non_skipped = [r for r in predecessors if r.schedule_relationship != ScheduleRelationship.SKIPPED]
    if non_skipped:
...
def _pick_successor_carry_forward(
    ...
    if not successors:
        return None
    non_skipped = [r for r in successors if r.schedule_relationship != ScheduleRelationship.SKIPPED]
...
class RealtimeScheduleRepo:
    """RealtimeScheduleRepo repository."""
...
async def provide_schedule_update_repo(
    ...
    return RealtimeScheduleRepo(session=db_session)
