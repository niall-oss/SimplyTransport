...
from .shape_model import ShapeGeometryRow, ShapeModel


class ShapeRepo(SQLAlchemyAsyncRepository[ShapeModel]):  # type: ignore[type-var]
    """Stop repository."""
...
async def provide_shape_repo(db_session: NamedDependency[AsyncSession]) -> ShapeRepo:
    """This provides the Shape repository."""

    return ShapeRepo(session=db_session)
